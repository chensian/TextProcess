...
voca_path = "G:/Stock/output/model/"
extend_words_path = "G:/Stock/output/wordlist/extension/"
...
def wordlist_wm_value(name, word_dict):
    ...
    wordlist = file_to_list(extend_words_path, "extend_" + word_dict)
    voca_freq = file_to_dict(voca_path, "vocabulary")
    ...
    #  加载 lda 词统计库
    filename = name + ".lda-c"
    data = codecs.open(path + filename).readlines()
...
def merge_wordlist_wm_value(name):
    file2id = pd.read_csv("G:/Stock/output/seg/file2id.csv")
    ...
    for word_dict in word_dicts:
        ...
        file2id.to_csv("G:/Stock/output/statistic/wm_value_" + word_dict + ".csv", index=False)
